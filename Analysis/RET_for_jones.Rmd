...
rm(list=ls())
x <- c("ggmap", "sf", "tidyverse", "foreign", 'forcats', 'censusapi', 'stringr', 'viridis', 'rvest', 'ggplot2') # , "tmap", "rgdal", "rgeos", "maptools", "tidyr"
...
# I was initially thinking there'd be separate tables for each scale of analysis: region, PUMA and income bin within each PUMA. I now think it might just have been better to have one big dataframe at the PUMA & income bin level
######################################## PUMA Table #####################################
...
So actually, let's just assume that the next five years of construction (2016-2020) will follow the patterns of the last 5. The California Department of Finance projects that the state will add `r as.integer(40719999-39059809)` new residents in `r as.integer(13864699-13236154)` households.
...
big_table %>% ungroup() %>% group_by(inc_bin) %>% summarize(`Households: Market Infill (1,000s)` = sum(infill_mkt_rate_hh) / 1000,
  ...
  mutate_at(.vars = vars(`Per-HH Emissions: Market Infill`, `Per-HH Emissions: Affordable Infill`),
            .funs = funs(round(. ,1))) %>%
  rbind(c("Total",as.list(unlist(t(colSums(.[,-1])))[1:6]))) %>%
  kable(format = 'markdown', format.args = list(big.mark = ','), caption = "Summary Results")
```

If the highest income households are placed in the lowest HH GHG areas:
[Table that breaks out total GHGe by income bracket, summed up across the state, with a total row at the bottom]
